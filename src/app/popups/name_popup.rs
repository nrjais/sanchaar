...
use crate::commands::builders::{
    ...
    rename_request_cmd, rename_script_cmd,
};
    ...
    pub fn update(self, state: &mut AppState) -> Task<Message> {
        match self {
            Message::NameChanged(name) => {
                let Some(Popup::PopupName(data)) = state.common.popup.as_mut() else {
                    return Task::none();
                };
                data.name = name;
                Task::none()
            ...
            Message::Rename(name) => {
                let action = {
                    let Some(Popup::PopupName(data)) = state.common.popup.as_ref() else {
                        return Task::none();
                    };
                    data.action.clone()
                };
                ...
                match action {
                    PopupNameAction::RenameCollection(col) => {
                        state.common.collections.rename_collection(col, name);
                        Task::done(Message::Done)
                    }
                    PopupNameAction::RenameFolder(col, folder_id) => {
                        rename_folder_cmd(&mut state.common, col, folder_id, name)
                            .map(|_| Message::Done)
                    }
                    PopupNameAction::CreateFolder(col, folder_id) => {
                        create_folder_cmd(&mut state.common, col, folder_id, name)
                            .map(|_| Message::Done)
                    }
                    PopupNameAction::RenameRequest(col, req) => {
                        rename_request_cmd(&mut state.common, CollectionRequest(col, req), name)
                            .map(|_| Message::Done)
                    }
                    PopupNameAction::NewRequest(col, folder) => create_new_request_cmd(
                        &mut state.common,
                        col,
                        folder,
                        name,
                    ...
                    PopupNameAction::NewScript(col) => {
                        create_script_cmd(&mut state.common, col, name).map(|_| Message::Done)
                    }
                    PopupNameAction::RenameScript(col, old_name) => {
                        rename_script_cmd(&mut state.common, col, old_name, name)
                            .map(|_| Message::Done)
                    }
