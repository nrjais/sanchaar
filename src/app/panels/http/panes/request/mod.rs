...
use crate::components::{
    ...
    key_value_editor, tooltip, vertical_button_tabs,
};
...
fn script_view<'a>(
    ...
    use crate::components::{ContentType, code_editor};
    ...
    let script_type_tabs = vertical_button_tabs(
        script_type,
        [
            button_tab(ReqScriptType::PreRequest, || text("Pre-request")),
            button_tab(ReqScriptType::PostRequest, || text("Post-response")),
        ]
        .into_iter(),
        RequestPaneMsg::ChangeScriptType,
    );

    let script_selector = Column::new()
        .push(
            Row::new()
                ...
                .push(
                    pick_list(
                        ...
                        move |s| match script_type {
                            ReqScriptType::PreRequest => {
                                RequestPaneMsg::ChangePreRequestScript(Some(s))
                            }
                            ReqScriptType::PostRequest => {
                                RequestPaneMsg::ChangePostRequestScript(Some(s))
                            }
                        },
                    )
            ...
            Row::new()
                ...
                .push(tooltip(
                    ...
                    icon_button(icons::Close, Some(18), Some(8))
                        .on_press_maybe(selected.map(|_| match script_type {
                            ReqScriptType::PreRequest => {
                                RequestPaneMsg::ChangePreRequestScript(None)
                            }
                            ReqScriptType::PostRequest => {
                                RequestPaneMsg::ChangePostRequestScript(None)
                            }
                        }))
                        .style(button::secondary),
                ))
    ...
    let editor_view: iced::Element<'a, RequestPaneMsg> = if let Some(script_name) = selected {
        if let Some(script_content) = request.script_content.as_ref() {
            let save_button = if request.script_edited {
                button("Save Script")
                    .padding([6, 16])
                    .on_press(RequestPaneMsg::SaveScript)
            ...
            Column::new()
                .push(
                    container(
                        Row::new()
                            .push(text(format!("Editing: {}", script_name)).size(14))
                            .push(space::horizontal())
                            .push(save_button)
                            .spacing(12)
                            .align_y(iced::Alignment::Center)
                    ...
                    container(
                        scrollable(
                            code_editor(script_content, ContentType::JavaScript)
                                .editable()
                                .map(RequestPaneMsg::ScriptEditorAction),
                        )
                        .height(Length::Fill),
                    )
            ...
            container(text("Loading script...").size(14))
                .padding(12)
                .center(Length::Fill)
                .into()
        }
    } else {
        container(text("Select a script to edit").size(14))
            .padding(12)
            .center(Length::Fill)
            .into()
    };
